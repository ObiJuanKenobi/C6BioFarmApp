...
class BuyViewController: UIViewController {
    ...
    @IBAction func cropUpdate(sender: AnyObject) {
        ...
        switch seg_CropChoice.selectedSegmentIndex {
            ...
            if(farmInBuyView!.getYear() != 1){
                lbl_OtherCropInfo.text = String(format: "This field had \(farmInBuyView!.fields[selectedFarm].getOldCrop()) last year")
            }
            else{
            ...
            swt_Insur.on = true
             ...
             //sets what C6 will say in his text bubbles
            if(farmInBuyView!.getYear() != 1){
                lbl_OtherCropInfo.text = String(format: "You planted \(farmInBuyView!.fields[selectedFarm].getOldCrop) last year")
            }
            else{
                lbl_OtherCropInfo.text = "Remember to rotate your crops!"
            }
    ...
    */
    func updateCalculation () {
        ...
        lbl_Calculation.text = String(format: "%@: \nnumber of acres %.0f * \nprice of crop %.2f\n Market Price: %.2f", name, farmInBuyView!.fields[selectedFarm].getLandSize(), cost, cropSellingPrice)
        
        var calculatedCost = cost * Float (farmInBuyView!.fields[selectedFarm].getLandSize())
