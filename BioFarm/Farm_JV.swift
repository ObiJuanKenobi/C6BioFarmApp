...
//the enums for the Crop has Insured corn and insured soybeans as a crop to make calcuation easier
...
class FarmJV {
    ...
    var modifierNumber: Float
        ...
        init(){
            ...
            previousYearFields = [Crop.Empty,Crop.Empty,Crop.Empty,Crop.Empty,Crop.Empty,Crop.Empty,Crop.Empty,Crop.Empty]
            fields = [Crop.Empty,Crop.Empty,Crop.Empty,Crop.Empty,Crop.Empty,Crop.Empty,Crop.Empty,Crop.Empty]
...
*/
    func getCostOfPurchase(whichField: Int, whichCrop: Crop) -> Float{
        switch whichCrop{
        case .Corn:
...
*/
        func plant(whichField: Int, whichCrop: Crop){
...
*/
        ...
        func eventGenerator(){
            ...
            default:
                print("Something went wrong in eventGenerator")
            }
        }
        /*
    function that calculates the yield of the farm
    if there was a flood then the yield is calculated differently
    also negatively impacts teh yield if they did not rotate their Crop
*/
        func calculateYield(){
            if(floodingEvent){
                floodingEvent = false;
                for var i = 6; i < 8; i++ {
                    ...
                    else if (fields[i] == Crop.InsuredCorn){
                        fieldYields[i] = (160 * Float(fieldSizes[i])) * 0.75
                    }
                    else if (fields[i] == Crop.InsuredSoybean){
                ...
                for(var i = 0; i < 8; i++){
                    ...
                    else if(fields[i] == Crop.InsuredCorn && modifierNumber == 1.0){
                        fieldYields[i] = (160.0 *  Float(fieldSizes[i]))
                    }
                    else if (fields[i] == Crop.InsuredCorn && modifierNumber != 1.0){
                        fieldYields[i] = (160.0 * 0.75) * Float(fieldSizes[i])
                    }
                    else if(fields[i] == Crop.InsuredSoybean && modifierNumber == 1.0){
                        fieldYields[i] = 45.0 * Float(fieldSizes[i])
                    }
                    else if(fields[i] == Crop.InsuredSoybean && modifierNumber != 1.0){
                        fieldYields[i] = (45.0 * 0.75) * Float(fieldSizes[i])
                    }
                    else if (fields[i] == Crop.Corn && modifierNumber != 1.0){
                    ...
                    else if(fields[i] == Crop.Soybean && modifierNumber != 1.0){
                        if(modifierNumber == 0.0){
                            modifierNumber = 1.0
                        }
...
*/
        func calculateRevenue(){
            for var i = 0; i < 8; i++ {
                ...
                else if (fields[i] == Crop.Corn || fields[i] == Crop.InsuredCorn){
                    totalRevenue = totalRevenue + (Float(fieldYields[i]) * Float(cornSellingPrice))
                }
                else if(fields[i] == Crop.Soybean || fields[i] == Crop.InsuredSoybean){
